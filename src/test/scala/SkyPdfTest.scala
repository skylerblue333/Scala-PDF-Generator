...
class SkyPdfTest extends munit.FunSuite:
  ...
  test("rejects unsupported Unicode and oversized documents") {
    assertEquals(SkyPdf.render(SkyPdf.Document("Sky 🌌", Vector.empty)).left.toOption, Some("only printable ASCII is supported"))
    assertEquals(SkyPdf.render(SkyPdf.Document("Sky", Vector.fill(41)("line"))).left.toOption, Some("too many lines"))
  }
